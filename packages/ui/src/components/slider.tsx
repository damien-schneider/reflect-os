...
function Slider({
  ...
  const _values = React.useMemo(
    () =>
      Array.isArray(value)
        ? value
        : // biome-ignore lint/style/noNestedTernary: simple conditional for default value handling
    ...
    >
      ...
      >
        <SliderPrimitive.Track
          className="relative select-none overflow-hidden rounded-md bg-muted data-horizontal:h-3 data-vertical:h-full data-horizontal:w-full data-vertical:w-3"
        ...
        {Array.from({ length: _values.length }, (_, index) => (
          <SliderPrimitive.Thumb
            ...
            // biome-ignore lint/suspicious/noArrayIndexKey: slider thumbs are static and based on value array length
            key={index}
          />
        ))}
      </SliderPrimitive.Control>
